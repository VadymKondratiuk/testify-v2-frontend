  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm p-6">
      ...
      <div className="flex flex-col gap-4">
        {history.map((item) => (
          <div key={item.id} className="flex items-center justify-between py-2 border-b border-[#F1F5F9] last:border-0 last:pb-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[0.9rem] font-semibold text-[#0F172A]">{item.title}</div>
                <div className="text-[0.75rem] text-[#94A3B8]">{item.date}</div>
              </div>
            </div>
            <div className={`font-bold text-[0.9rem] ${item.passed ? 'text-[#10B981]' : 'text-[#EF4444]'}`}>
              {item.score}
            </div>
          </div>
        ))}
